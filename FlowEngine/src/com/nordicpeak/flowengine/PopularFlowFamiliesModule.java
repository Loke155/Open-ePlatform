...
public class PopularFlowFamiliesModule extends AnnotatedBackgroundModule implements EventListener<CRUDEvent<Flow>>, Runnable{
	...
	private void cacheFlows(){
		...
		try{
			...
			List<Integer> familyIDs = new ArrayListQuery<Integer>(dataSource, true, "SELECT DISTINCT flowFamilyID FROM flowengine_flows WHERE publishDate <= CURDATE() AND (unPublishDate IS NULL OR unPublishDate > CURDATE());", IntegerPopulator.getPopulator()).executeQuery();
