var mapClientScriptLocation;var mapClientScriptLocationFallback;var mapClientLanguages={"DIMENSION_AND_ANGLE_SETTINGS":"Dimension and angle settings"};var mapInstances={};var version="1.2.0";var LoadFiles=function(files,callback){var fileCounter=0;var file;var obj;var cssFilesIndex=document.styleSheets.length-1;for(index in files){var file=files[index];fileCounter++;if(getFileType(file)=="css"){addCssFile(file);cssFilesIndex++;if(!window.opera&&navigator.userAgent.indexOf("MSIE")==-1){checkCSSLoadingState(cssFilesIndex)
}}else{addJsFile(file)}}function getFileType(file){file=file.toLowerCase();var jsIndex=file.indexOf("js"),cssIndex=file.indexOf("css");if(jsIndex==-1&&cssIndex==-1){return false}return jsIndex>cssIndex?"js":"css"}function addCssFile(url){var link=document.createElement("link");link.href=url;link.rel="stylesheet";link.type="text/css";link.onload=checkFinishedState;link.onreadystatechange=function(){if(this.readyState=="loaded"||this.readyState=="complete"){checkFinishedState()}};document.getElementsByTagName("head")[0].appendChild(link)
}function addJsFile(file){var script=document.createElement("script");script.type="text/javascript";script.src=file;script.onload=checkFinishedState;document.getElementsByTagName("head")[0].appendChild(script)}function checkCSSLoadingState(index){try{if(document.styleSheets[index].cssRules){checkFinishedState()}else{if(document.styleSheets[index].rules&&document.styleSheets[index].rules.length){checkFinishedState()}else{setTimeout(function(){checkCSSLoadingState(index)},250)}}}catch(e){setTimeout(function(){checkCSSLoadingState(index)
},250)}}function checkFinishedState(){fileCounter--;if(fileCounter==0){callback();fileCounter=-1}}};(function(){if(!$("html").hasClass("map-client-riges")){Ext={buildSettings:{"scopeResetCSS":true}};var scriptNameRaw="map-client-riges.min.js";var mapClientScriptName=new RegExp("(^|(.*?\\/))("+scriptNameRaw+")(\\?|$)");var scripts=document.getElementsByTagName("script");for(var i=0;i<scripts.length;i++){var src=scripts[i].getAttribute("src");if(src){var match=src.match(mapClientScriptName);if(match){mapClientScriptLocation=match[1];
break}}}if(mapClientScriptLocation==undefined&&mapClientScriptLocationFallback!=undefined){mapClientScriptLocation=mapClientScriptLocationFallback.match(mapClientScriptName)[1]}var cssFiles=new Array(mapClientScriptLocation+version+"/ext/resources/css/ext-all-oep.css",mapClientScriptLocation+version+"/resources/OpenEMap.css",mapClientScriptLocation+"css/map-client-riges.css",mapClientScriptLocation+version+"/openlayers/theme/default/style.css");var scripts=new Array(mapClientScriptLocation+version+"/ext/ext-all.js",mapClientScriptLocation+version+"/ext/ext-theme-oep.js",mapClientScriptLocation+version+"/ext/locale/ext-lang-sv_SE.js",mapClientScriptLocation+version+"/openlayers/OpenLayers.js",mapClientScriptLocation+version+"/geoext2-all.js",mapClientScriptLocation+version+"/es5-shim.min.js",mapClientScriptLocation+version+"/OpenEMap-all.js");
LoadFiles(cssFiles,function(){loadScripts(scripts)});$("html").addClass("map-client-riges")}})();function loadScripts(jsFiles){var scriptTags=new Array(jsFiles.length);for(var i=0;i<jsFiles.length;i++){scriptTags[i]="<script src='"+jsFiles[i]+"' type='text/javascript'><\/script>"}if(scriptTags.length>0){var $head=$("head");for(var i=0,len=scriptTags.length;i<len;i++){$head.append(scriptTags[i])}}}function MapClientInstance(queryID,mapID){this.MAP_RESOLUTIONS=[256,128,36,12,4,2,1,0.5,0.25,0.125];this.mapID=mapID;
this.mapDiv=$("#"+mapID);this.map;this.featureDialogs={};this.dialog;this.preview;this.externalGraphicsLocation=mapClientScriptLocation+"images";this.mapImageLocation=mapClientScriptLocation+version+"/resources/images/";this.config;this.originalTools;this.layerMapping={};this.enableEdgeLabeling=false;var instance=this;mapInstances[queryID]=instance;instance.setCustomConfig=function(config,customConfig,preview){if(customConfig.extent){config.extent=customConfig.extent}if(customConfig.search&&config.gui.searchFastighet){config.gui.searchFastighet={renderTo:customConfig.search.renderTo};
...
instance.beforeFeatureAddedCallback=function(e,instance){};instance.sketchCompletedCallback=function(e,instance){};instance.mapLoadedEventCallback=function(object,instance){};instance.layerVisibilityChangedCallback=function(e,instance){};instance.baseLayerChangedCallback=function(e,instance){};return instance}function waitUntilMapIsCreated(mapInstance,config,customConfig,preview){if(!(typeof OpenEMap==="undefined")&&!(typeof OpenEMap.Client==="undefined")){if(customConfig){mapInstance.setCustomConfig(config,customConfig,preview)
}mapInstance.initMapClient(config)}else{setTimeout(function(){waitUntilMapIsCreated(mapInstance,config,customConfig,preview)},5)}};